...
# WHY DO THEY DO THIS SORT OF THING? IF "YOU" ARE "THEY", PLEASE STOP DOING IT!
  ...
  deparse1 <- function(x) {
    if( is.call( x) && identical( x[[1]], as.name( ':=')) && length( x)==3) {
      xl <- deparse( x[[2]], width.cutoff = 500)
      xr <- deparse( x[[3]], width.cutoff = 500)
      x <- xl %&% ' := ' %&% xr
    } else {
  ...
  if ("debug" %in% loadedNamespaces()) {
    debuggify.system.call <- function(step.check = FALSE, nlocal = sys.parent()) mlocal(
  ...
  src.expr <- do.call("expression", list(src.expr[[c(1, length(src.expr[[1]]) - 1)]]))
  if (using.parser) {
    id <- ld$id
  ...
  while (length(i)) {
    ...
    if (mode(expr[[i]]) == "(" || !is.call(expr[[i]])) {
      ...
      `<-` = {
          temp.expr <- expr[[i]]
          temp.expr[[3]] <- 0
          temp.expr <- paste( deparse( temp.expr, width.cutoff = 500), collapse = " ")
